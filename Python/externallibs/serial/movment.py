def setPositon(vector, movefunc):
    ...
    [ordem[v['channel']].append([v["axis"], v['coordinate']] if not v["await"] else (v["axis"], str(v['coordinate']))) for v in vector]

    # Limpra as posições que não foram usadas.
    ordem = list(filter(None, ordem))

    # Executa a função de movimentação passando as posições como *args, usa define se 'nonsync',
...
movments ={
    "get":[
        ...
        [
            ...
            {"axis": "D", "coordinate": 70.0, "channel": 3, "await":True },
            {"axis": "C", "coordinate": 80.0, "channel": 4, "await":False}
        ]   
    ]
}
